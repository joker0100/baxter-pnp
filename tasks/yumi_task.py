...
class PickAndPlace:
    ...
    def _approach(self, pose, arm):
        ...
        check =arm.is_pose_reachable(RigidTransform(translation=approach[:3], rotation = self.rpy_to_wxyz(pose[3]*180/pi +90, 0, 180), from_frame='gripper', to_frame='world'))
        
        print('check :', check)
        if not check:
            return

        arm.goto_pose(RigidTransform(translation=approach[:3], rotation = self.rpy_to_wxyz(pose[3]*180/pi +90, 0, 180), from_frame='gripper', to_frame='world'))
    ...
    def run(self):
        ...
        self.robot.right.close_gripper(force= 20, width=0)
        self.robot.left.close_gripper(force= 20, width=0)
